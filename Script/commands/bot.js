...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "MAHIM ISLAM",
  description: "goibot with external API",
...
module.exports.handleEvent = async function({ api, event, Users }) {
  // 🛡️ Safety check: If event has no body (like images/stickers), ignore it
  ...
  // 🎯 THE "BOT" COMMAND: Fetches random text from your custom PHP API
  if (text === "bot") {
    let name = "Babu"; // Default fallback name
    try {
      ...
      // Assuming your PHP returns JSON like: {"status": "success", "data": "Random text here"}
      // If your API returns the string directly, use `response.data` instead of `response.data.data`
      ...
      // Fallback message if your API is down
      const fallbackMsg = {
