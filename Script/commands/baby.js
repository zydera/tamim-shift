...
// 🚀 OPTIMIZATION: Arrays stay outside for speed. Combined into one list.
const triggers = [
  "baby", "bby", "pakhi", "jan", "xan",
  "babu", "moyna", "sona", "janu", "jaan",
...
const greetings = [
  ...
  "~কার জন্য এতোগামী মায়া...!😌🥀 ~এই শহরে আপন বলতে...!😔🥀 ~শুধুই তো নিজের ছায়া...!😥🥀",
...
module.exports.handleEvent = async function ({ api, event, Users }) {
  try {
    ...
    for (const t of triggers) {
      // Regex correctly matches the word separated by spaces or at string start/end
    ...
    if (matchedTrigger) {
      // Remove the trigger word from the sentence so the API only receives the actual question
      const regex = new RegExp(`(^|\\s)${matchedTrigger}(\\s|$)`, 'i');
      const query = raw.replace(regex, ' ').trim();
      
      // If there is still a query left after removing the trigger, send it to the API
      if (query) {
        await fetchAndSendSimSimi(api, event, query, senderName);
      }
    }
